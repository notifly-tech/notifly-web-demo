...
| 장바구니 리마인드 | 웹 푸시 | `product_added_to_cart` | 상품을 담은 뒤 브라우저 알림 허용 | “장바구니에 담긴 상품이 기다리고 있어요.” |
| 구매 후 리뷰 유도 | 웹 푸시 | `purchase_completed` | 결제 완료 후 브라우저 알림 허용 | “상품을 받아보신 뒤 리뷰를 남겨주세요.” |
| 뉴스레터 환영 | 웹 팝업 또는 웹 푸시 | `newsletter_subscribed` | 페이지 하단에서 이메일 구독 | “구독이 완료되었습니다. 이번 주 큐레이션을 보내드릴게요.” |

웹 푸시는 사용자가 브라우저 알림을 허용해야 받을 수 있습니다. 먼저 마이페이지에서 `푸시 권한 요청`을 눌러 권한을 허용한 뒤 테스트하세요.

### 웹 팝업 캠페인 만드는 방법

1. Notifly 콘솔에서 웹 팝업 캠페인을 새로 만듭니다.
2. 트리거를 “이벤트 발생 시”로 선택합니다.
3. 이벤트 이름에 위 표의 값을 입력합니다. 예: `product_added_to_cart`
4. 팝업 제목과 본문을 입력합니다.
5. 캠페인을 저장하고 활성화합니다.
6. 데모 앱에서 해당 행동을 실행합니다. 예: 상품 카드의 `장바구니 담기` 클릭
7. 화면에 팝업이 보이는지 확인합니다.

처음에는 조건을 단순하게 두는 편이 좋습니다. 팝업 노출이 확인된 뒤 `cart_value`, `page`, `membership_tier` 같은 조건을 하나씩 추가하면 원인을 찾기 쉽습니다.

### 이벤트 기반 웹 푸시 캠페인 만드는 방법

1. 데모 앱을 HTTPS로 실행합니다. 예: `npm run dev:https`
2. 마이페이지에서 `푸시 권한 요청`을 누르고 브라우저 알림을 허용합니다.
3. Notifly 콘솔에서 웹 푸시 캠페인을 새로 만듭니다.
4. 발송 방식을 “이벤트 기반” 또는 “이벤트 발생 시 발송”으로 선택합니다.
5. 이벤트 이름을 입력합니다. 예: `purchase_completed`
6. 푸시 제목과 내용을 입력합니다.
7. 캠페인을 저장하고 활성화합니다.
8. 데모 앱에서 해당 행동을 실행합니다. 예: 체크아웃에서 결제 완료
9. 브라우저 알림이 도착하는지 확인합니다.
